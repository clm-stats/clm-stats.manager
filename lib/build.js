...
const mkDefaultRunner =
  ...
  async (...args) => {
    try {
      log.info(`executing build step ![${name}]`);
...
async function buildNewActions(newActions = []) {
  ...
  log.info("managing result via git");
  const isUnchanged = await gitIsClean(paths.pagesCO());
  if (isUnchanged) {
    log.info("build did not produce any changes. done now");
    return;
  }
  ...
  log.info("complete build pushed to github");
  log.info("view deploy status here:");
  log.info("     https://github.com/clm-stats/clm-stats.github.io/actions");
  const selfHash = await lastCommit(paths.pagesCO());
  log.info(`for commit: ${selfHash}`);
  return selfHash;
}
...
export default new (class BUILDER {
  ...
  assertBuildable() {
    ...
    if (this.lastFinish && Date.now() - this.lastFinish < 1000 * 6 * 5) {
  ...
  startBuild(newActions) {
    ...
    return (async () => {
      try {
        ...
        this.lastHash = await buildNewActions(newActions);
        this.error = null;
      } catch (e) {
        this.error = e;
      } finally {
