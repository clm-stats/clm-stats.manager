...
Step("save-built-data", ["build-full-data", "build-managed-state"])();
Step("build-pages-app", ["save-built-data", "build-managed-state"])();
...
async function buildNewActions(newActions = [], logFn) {
  ...
  if (isDevEnv()) {
    X.logInfo("dev environment does not handle commits");
    return;
  }
