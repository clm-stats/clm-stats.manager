...
const runGqlQuery = (token) => async (qname, vars) => {
  ...
  const cached = await ((async () => {
    try { return [JSON.parse(await fs.readFile(qpathCached, 'utf8'))] }
...
runGqlQueryGGAuth.disableNetwork = () => { cacheOnly = true; };
runGqlQueryGGAuth.enableNetwork = () => { cacheOnly = false; };
