...
export default async function BuildManagedState(X) {
  const actions = await _.slurpOr(paths.data.state("manageActions.json"), null);
  if (!actions) {
    throw "could not load manageActions json file";
  }

  const eventAutoActions = {
    "tournament/.*/event/.*": (slug) => A.AddEvent(slug),
    "tournament/fudds.*": (slug) => A.MakeIneligible(slug),
