...
async function getClmEvents(startTime, endTime, logInfo = () => {}) {
  const after = startTime;
  const before = endTime;
  const data = await R.ggQueryAll(
    "clmEvents",
    { after, before },
    {
      page: {
...
export default async function BuildManagedState(X) {
  ...
  const now = 60 * 60 * Math.floor(Date.now() / 1000 / 60 / 60) + 24 * 60 * 60;
  X.logInfo(`getting clm events as of ${now}`);
  const periodStartTime = consts.getPeriodStartTime(14);
  const latestSlugs = await getClmEvents(periodStartTime, now, X.logInfo);
