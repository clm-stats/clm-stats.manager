...
export default async function BuildLegacyData() {
  const EVENTS = {};
  const ALL_IDENTS = {};
  const PLAYERS = {};
  async function addPlaywrightCrawlData() {
    ...
    for (const periodId in periods) {
      ...
      for (const player in players) {
        playerIdentByName[players[player].name] = player || "Kill All Grownups";
      }
  ...
  async function addDirectDLData() {
    ...
    for (const playerIdent in dataByIdent) {
      for (const periodId in dataByIdent[playerIdent]) {
        const trueIdent = playerIdent || "Kill All Grownups";
        PLAYERS[trueIdent] ||= {};
        PLAYERS[trueIdent][periodId] = [
  ...
  const IDENT_CLM_IDS = {};
  const RESERVED = await _.slurp(paths.v2Utils("reservedClmIds.json"));
