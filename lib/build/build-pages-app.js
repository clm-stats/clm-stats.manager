...
export default async function BuildPagesApp(X) {
  ...
  await Promise.all(
    timeline.periods.map(async (period) => {
      const { periodId } = period;
      const isCurrent = `${periodId}` === `${spec.currentPeriodId}`;
      ...
      await writeLayout("compare");
      await writeLayout("h2h");
      await writeMd("stats");
      await writeMd("players");
      await writeMd("compare");
      await writeMd("h2h");
      await writeMd("stats", "index");
      await writeMd("players", "-");
      await writeMd("stats", "index", isCurrent);
      await writeMd("players", "-", isCurrent);
      await writeMd("compare", "compare", isCurrent);
      await writeMd("h2h", "h2h", isCurrent);
    }),
  );
  await buildStatic(X);
