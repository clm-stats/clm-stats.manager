...
export default async function BuildPagesApp(X) {
  ...
  await build({ configFile: paths.appCO("vite.client.js") });
  const jsStr = await _.readStr(paths.npmRoot("dist", "mountClient.js"));
  await X.writeStr(paths.pagesDocs("index.js"), jsStr);
  await build({ configFile: paths.appCO("vite.ssr.js") });
  const { createDocString } = await import(paths.npmRoot("dist", "ssrApp.cjs"));
  await Promise.all(
    timeline.periods.map(async (period, periodInd) => {
      ...
      const writeLayout = (page) =>
        X.writeStr(
          layoutPath(page),
          createDocString(render, timeline, periodId, page),
        );
      const mkMd = (page) =>
