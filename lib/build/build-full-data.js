...
export default async function BuildFullData(X) {
  ...
  for (const slug of slugs) {
    ...
    for (const entrant of event.entrants.nodes) {
      ...
      const prEligible = !spec.ineligibleSlugs.has(event.slug);
      EVENT.prEligible = prEligible;
      ...
      for (const participant of entrant.participants) {
        ...
        getRankData(PLAYER.clmId).prEventSlugs.add(slug);
        ...
        PLAYER.pronouns ||= user.pronouns || player.genderPronoun;
  ...
  for (const setIdGroup of setIdGroups) {
    ...
    for (const setId of setIdGroup) {
      ...
      const { prEligible, slug } = PERIOD.events[eventId];
      ...
      if (!prEligible) {
        console.log(JSON.stringify({ s: slug, prE: prEligible }));
      }
      if (!set.doesCount || !prEligible) {
        continue;
      }
      anyCount = true;
  ...
  for (const ident in PERIOD.players) {
    ...
    RANK.conservativeRating = ratings.length < 3 ? -Infinity : trueRate;
