...
function getPeriodIdByDate(date) {
  const periodYearContrib = (date.getFullYear() - 2022) * 3 + 2;
  const periodMonthContrib = Math.floor(date.getMonth() / 4);
  return periodMonthContrib + periodYearContrib;
}

const activePeriodId = getPeriodIdByDate(new Date());

function getPeriodProps(periodId) {
  // only works for periodId > 10
  const year = Math.floor((periodId + 1) / 3) + 2021;
  const actNum = ((periodId + 1) % 3) + 1;
  const title = `${year} Act ${actNum}`;
  const season = `chicago_${year}-${actNum}`;
  const isActive = activePeriodId === periodId;
  return { title, isActive, season, timelineInd: periodId - 2 };
}

export async function buildFullPeriod(
  legacyData,
  periodId,
  nameDataByPlayerId,
  events,
  ...
  const D = legacyData;
  const { title, season, isActive, timelineInd } = getPeriodProps(periodId);
  D.timeline.push({ periodId, isActive, title, timelineInd, season });
  const isAll = false;
  ...
  slugs
    .filter((s) => events[s])
    .sort((s1, s2) => events[s1].date - events[s2].date);
  const setIdGroups = [];
  ...
  function getSortedEventIds(clmId) {
    ...
    function sortVal(id) {
      const event = PERIOD.events[id];
      return event ? event.date : 0;
    }
  ...
  const periodInds = Object.keys(D.periods).map((s) => parseInt(s));
  periodInds.sort((p1, p2) => p2 - p1);
  ...
  for (const slug of slugs) {
    ...
    function nameDataFromParticipant(participant) {
      ...
      return (() => {
        ...
        const baseNameData = nameDataByPlayerId[playerId] || {
          name,
          ident: name,
        };
        nameDataByPlayerId[playerId] = baseNameData;
        return { ...baseNameData, otherIdent: name };
      })();
    }
    ...
    EVENT.isChallonge = spec.challongeSlugs.has(slug);
    EVENT.tournamentName =
      spec.tournamentNameOverrides[slug] || event.tournamentName;
    ...
    for (const entrant of Object.values(event.entrants)) {
      for (const participant of entrant.participants) {
        ...
        const { ident, isMask, otherIdent } = nameData;
        if (isMask && !PERIOD.players[ident]) {
          for (const periodInd of periodInds) {
            PERIOD.players[ident] ||= D.periods[periodInd].players[ident];
          }
        }
        const PLAYER = PERIOD.players[ident] || {};
        if (otherIdent && ident !== otherIdent) {
          identAliases[otherIdent] = ident;
          PLAYER.otherIdents ||= new Set([]);
          PLAYER.otherIdents.add(otherIdent);
        }
    ...
    for (const entrant of Object.values(event.entrants)) {
      const standing = entrant.standing;
      if (!standing) {
        continue;
      }
      const clmId = clmIdByEntrantId[entrant.id];
  ...
  for (const setIdGroup of setIdGroups) {
    ...
    for (const setId of setIdGroup) {
      ...
      const overrides = spec.setOverrides[setId] || {};
      const winnerId = overrides.winnerId || set.winnerId;
      const [w, l] = winnerId === e1.id ? [e1, e2] : [e2, e1];
  ...
  function getSetInfo(setId, forId) {
    ...
    const [wonGames, lostGames] = (() => {
      if (overrides.games) {
        ...
        return wonH2hSet ? [games.w, games.l] : [games.l, games.w];
      }
      return wonH2hSet === is1w
        ? [set.slots[0].displayScore, set.slots[1].displayScore]
        : [set.slots[1].displayScore, set.slots[0].displayScore];
    })();
    ...
    if (winnerId !== set.winnerId) {
      setInfo.isOverridden = true;
    }
  ...
  for (const ident in PERIOD.players) {
    ...
    for (const oppId of getH2hSortedOppIds(clmId)) {
      ...
      for (const setId of getH2hRankData(clmId, oppId)) {
        ...
        h2hSet.isChallonge = event.isChallonge;
  ...
  for (const ident in PERIOD.players) {
    const PLAYER = PERIOD.players[ident];
    const otherIdents = PLAYER.otherIdents || new Set([]);
    PLAYER.otherIdents = [...otherIdents];
    ...
    const sortedEventIds = getSortedEventIds(clmId);
    PLAYER.RANK.eventId = (() => {
      for (
        let eventInd = sortedEventIds.length - 1;
        eventInd >= 0;
        eventInd--
      ) {
        const eventId = sortedEventIds[eventInd];
        if (
          getEventRankData(clmId, eventId).wins +
            getEventRankData(clmId, eventId).losses >
          0
        ) {
          return eventId;
        }
      }
      return sortedEventIds[sortedEventIds.length - 1];
    })();
    PLAYER.RANK.placingString = ordinal(
      getEventRankData(clmId, PLAYER.RANK.eventId).placing,
    );
  ...
  for (const alias in identAliases) {
    if (PERIOD.players[alias]) {
      continue;
    }
    const ident = identAliases[alias];
    PERIOD.players[alias] = PERIOD.players[ident];
    D.players[alias] ||= {};
    D.players[alias][periodId] = D.players[ident][periodId];
  }
  ...
  PERIOD.hasNoVisibleRanks = true;
  for (const rank of PERIOD.ranks) {
    if (rank.prEvents > 2) {
      PERIOD.hasNoVisibleRanks = false;
    }
  }
  ...
  for (const pid in D.players) {
    for (const periodId in D.players[pid]) {
      ...
      D.players[pid][periodId][1].sort(
        (t1, t2) =>
          (t1.event ? t1.event.date : 0) - (t2.event ? t2.event.date : 0),
      );
    }
  }
  ...
  for (const periodId in D.periods) {
    for (const pid in D.periods[periodId].others) {
      const latestOthers = D.periods[periodId].others[pid];
      const latestPeriodId = Array.isArray(latestOthers)
        ? latestOthers[0]
        : latestOthers;
...
export async function buildFullData(legacyData, events, spec, actions) {
  const nameDataByPlayerId = await _.slurp(
    paths.v2Utils("nameDataByPlayerId.json"),
  );
  let D = legacyData;
  const periodIds = [];
  const eventsByPeriodId = {};
  for (const eventSlug in events) {
    const event = events[eventSlug];
    if (!event) {
      continue;
    }
    const periodId = getPeriodIdByDate(new Date(event.date * 1000));
    if (!eventsByPeriodId[periodId]) {
      periodIds.push(periodId);
      eventsByPeriodId[periodId] = {};
    }
    eventsByPeriodId[periodId][eventSlug] = event;
  }
  periodIds.sort();
  for (const periodId of periodIds) {
    const periodEvents = eventsByPeriodId[periodId];
    D = await buildFullPeriod(
      D,
      periodId,
      nameDataByPlayerId,
      periodEvents,
  ...
  D.actions = A.ejectEphemerals(actions);
  return D;
}
