...
export async function buildFullPeriod(legacyData, periodId, nameDataByPlayerId, events, spec) {
  const D = legacyData;
  ...
  for (const slug of slugs) {
    ...
    function nameDataFromParticipant(participant) {
      ...
      return (() => {
        ...
        const baseNameData = nameDataByPlayerId[playerId] || { name, ident: name };
...
export async function buildFullData(legacyData, events, spec, actions) {
  const nameDataByPlayerId = await _.slurp(paths.v2Utils("nameDataByPlayerId.json"));
  let D = legacyData;
  const periodIds = [];
  ...
  for (const periodId of periodIds) {
    ...
    D = await buildFullPeriod(D, periodId, nameDataByPlayerId, periodEvents, spec);
  }
  D.actions = actions;
  return D;
}
