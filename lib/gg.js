...
export async function getEventData(slug, checkState, logInfo = () => {}) {
  try {
    const event = await getEventDataImpl(slug, { cacheOnly: true, logInfo });
    if (checkState && event.state !== "COMPLETED") {
      logInfo(
        "event",
      ...
      return await getEventDataImpl(slug, { skipCache: true, logInfo });
    }
    return event;
  } catch (_e) {
    return await getEventDataImpl(slug, { logInfo });
  }
}

export async function getClmEvents(startTime, endTime, logInfo = () => {}) {
